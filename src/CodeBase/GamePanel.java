...
public class GamePanel extends OSprite {
    ...
    // Leaderboard map (player name -> score)
    ...
    public void gameOver() {
        ...
        if (playerName != null && !playerName.trim().isEmpty()) {
            leaderboard.put(playerName.trim(), score);
            updateLeaderboard();
        }

        // Show total score and leaderboard
        String leaderboardDisplay = getFormattedLeaderboard();
    ...
    private void updateLeaderboard() {
        // Ensure the leaderboard is sorted and limited to the top 10
        Map<String, Integer> sortedLeaderboard = leaderboard.entrySet().stream()
                .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
                .limit(3)
        ...
        leaderboard.clear();
        leaderboard.putAll(sortedLeaderboard);
    }
    ...
    private class DoodleAnimationListener extends AnimationListener {
        ...
        public void postAction() {
            if (doodle.getY() > getHeight()) {
                ...
                if (doodle.getVelocity().x > DoodlePlayer.MOVING_VECTOR.x) {
                    doodle.turnRight();
                } else if (doodle.getVelocity().x < -DoodlePlayer.MOVING_VECTOR.x) {
                    doodle.turnLeft();
                }
